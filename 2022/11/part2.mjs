...
function run() {
  try {
    ...
    // TODO: read around least common multiple / modular arithmetic
    const LCM = monkeys.map((m) => m.divisibleBy).reduce((a, b) => a * b);

    for (let i = 0; i < 10_000; i++) {
      monkeys.forEach((m) => {
        m.items.forEach((item) => {
          const updatedItem = calculateWorryLevel(item, m.op, m.opArg) % LCM;
    ...
    // CORRECT EXAMPLE ANSWER: 2713310158
    // CORRECT ANSWER: 18170818354
  } catch (err) {
    console.error(err);
  }
}
